...
def equations(params: Tuple[float, float, float]) -> List[float]:
    """
    Calculate the system of equations for solving the parameters using fsolve method.
...
def jacobi_matrix(params: Tuple[float, float, float]) -> np.ndarray:
    ...
    R_100_sp, R_98_sp, R_97_sp = 0.602807385, 1.513372443, 0.599205152
    R_100_std, R_98_std, R_97_std = 0.601491655, 1.51137031, 0.598673698
    r_100_mix, r_98_mix, r_97_mix = 0.83866852, 1.628762881, 0.773076736
...
def solve_equations(initial_guess: Tuple[float, float, float] = (0.5, 0.5, 2.0)) -> np.ndarray:
    """
    Solve the system of equations using fsolve method.
    
    Args:
        initial_guess (Tuple[float, float, float]): Initial values for (φ_ref, β_sple, β_mix)
            Default is (0.5, 0.5, 2.0)
    
    Returns:
        np.ndarray: Solution array containing the optimal values for (φ_ref, β_sple, β_mix)
    ...
    Example:
        ...
        3
    """
    solution = fsolve(equations, initial_guess, fprime=jacobi_matrix)
    return solution
